...
document
  .querySelector("#products-review-form")
...
document
  ...
  ["description"].addEventListener("input", (ev) => {
    productsUpdateDescCountSpan(ev.target.value.length);
  });

function productsUpdateDescCountSpan(count) {
  if (count === undefined) {
    count = document.querySelector("#products.review-form")["description"].value
      .length;
  }
